...
class StudentDocumentController extends Controller
{
    ...
    public function getAll(Request $request)
    {
        ...
        $items = StudentDocument::select(
            ...
            DB::raw("(CASE WHEN document_file = NULL THEN ''
             ELSE CONCAT('".$this->uploadUrl."',document_file) END) AS document_file"),
        ...
        if ($request->document_name) {
            $items->where('student_document.document_name','LIKE',"%".$request->document_name."%");
        }
        ...
        if ($request->document_type_id) {
            $items->where('student_document.document_type_id', $request->document_type_id);
        }
    ...
    public function get($id)
    {
        ...
        $item = StudentDocument::select(
            ...
            ->leftJoin('document_type','document_type.id','=','student_document.document_type_id')
            ->first();
    ...
    public function add(Request $request)
    {
        ...
        if(($request->document_file != "") && ($request->document_file != 'null') && ($request->document_file != 'undefined')){
            $fileDocument = 'document-'.rand(10,100).'-'.$request->file('document_file')->getClientOriginalName();
    ...
    public function edit($id, Request $request)
    {
        ...
        $item->document_name = $request->has('document_name') ? $request->document_name : $item->document_name;
        $item->document_type_id = $request->has('document_type_id') ? $request->document_type_id : $item->document_type_id;
        $item->document_file = $pathDocument;
        $item->student_id = $request->has('student_id') ? $request->student_id : $item->student_id;
