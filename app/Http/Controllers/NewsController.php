...
class NewsController extends Controller
{
    ...
    public function getAll(Request $request)
    {
        ...
        ->leftJoin("news_category", "news_category.id", "=", "news.news_cate_id");
        ...
        if ($request->news_title) {
            $items->where('news.news_title','LIKE',"%".$request->news_title."%");
        }
        ...
        if ($request->news_detail) {
            $items->where('news.news_detail','LIKE',"%".$request->news_detail."%");
        }
    ...
    {
        $request->validate([
            'news_title as required',
        ]);

        $pathFile = null;
        if ($request->news_file != "" && $request->news_file != "null" && $request->news_file != "undefined") {
            $fileName = date("YmdHis")."_document_".rand(1, 10000).".".$request->file("news_file")->extension();
    ...
    public function edit($id, Request $request)
    {
        ...
        $item->news_title = $request->has('news_title') ? $request->news_title : $item->news_title;
        $item->news_detail = $request->has('news_detail') ? $request->news_detail : $item->news_detail;
        $item->pinned = $request->has('pinned') ? $request->pinned : $item->pinned;
        $item->news_cate_id = $request->has('news_cate_id') ? $request->news_cate_id : $item->news_cate_id;
