...
class CompanyController extends Controller
{
    ...
    public function getAll(Request $request)
    {
        ...
        $items = Company::select(
            ...
            DB::raw("(CASE WHEN namecard_file = NULL THEN ''
            ELSE CONCAT('".$this->uploadUrl."',namecard_file) END) AS namecard_file"),
            DB::raw("(CASE WHEN location_file = NULL THEN ''
            ELSE CONCAT('".$this->uploadUrl."',location_file) END) AS location_file"),
        ...
        if($request->includeAll){
            $items->addSelect('province.name_th as province_name','amphur.name_th as amphur_name','tumbol.name_th as tumbol_name');
            $items->leftJoin('province','province.province_id','=','company.province_id');
            $items->leftJoin('amphur','amphur.amphur_id','=','company.amphur_id');
            $items->leftJoin('tumbol','tumbol.tumbol_id','=','company.tumbol_id');
        }
        ...
        if ($request->name_th) {
            $items->where('company.name_th','LIKE',"%".$request->name_th."%");
        }

        if ($request->name_en) {
            $items->where('company.name_en','LIKE',"%".$request->name_en."%");
        }
    ...
    public function get($id)
    {
        ...
        $item = Company::select(
            ...
            ->where('company.id', $id)
            ->leftJoin('province','province.province_id','=','company.province_id')
    ...
    public function add(Request $request)
    {
        ...
        $pathLocation = null;
        if(($request->location_file != "") && ($request->location_file != 'null') && ($request->location_file != 'undefined')){
